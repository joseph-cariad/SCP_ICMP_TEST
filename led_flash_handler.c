...
/*
led_1_handler function is used to turn on/off user led 1(151).
...
{
    if (turn_on){
        Dio_WriteChannel(151, STD_HIGH);
    }else{
        Dio_WriteChannel(151, STD_LOW);
    }
}


/*
led_2_handler function is used to turn on/off user led 2 (152).
...
led_4_5_6_handler function is used to turn on/off user led 4, 5 & 6.
                                        ...
                                        Value       led 4(154)  led 5(282)  led 6(283)
...
{
    if (binary_code%2){
        Dio_WriteChannel(283, STD_HIGH); //Led 6(ON)
    }else{
        Dio_WriteChannel(283, STD_LOW);//Led 6(OFF)
    }
    ...
    binary_code=binary_code/2;
    if (binary_code%2){
        Dio_WriteChannel(154, STD_HIGH);//Led 4(ON)
    }else{
        Dio_WriteChannel(154, STD_LOW);//Led 4(OFF)
    }
}
