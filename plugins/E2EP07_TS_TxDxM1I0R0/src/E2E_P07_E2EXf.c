...
#include <E2E_P07_E2EXf.h>  /* E2EXf APIs for E2E Profile P07 */
...
/* Call P07 dependent check functions */
static FUNC(Std_ReturnType, E2E_CODE) E2EXf_Inv_P07Check
(
...
/*------------------- E2EXf_P07InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P07InPlace_Transformer
(
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      ...
      CONSTP2CONST(E2E_P07ConfigType, AUTOMATIC, E2E_CONST) ProfileCfgPtr =
          &(ConfigPtr->P07Config);
      ...
      /* set buffer length */
      BufferLength = (uint32)((uint32)InputBufferLength +  (uint32)( ((uint32)PXXConfigPtr->HeaderLength) / (uint32)8U ));
      *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE) BufferLength;

      /* call protection function */
      RetValProt = E2E_P07Protect(ProfileCfgPtr, ProtectStatePtr, BufferPtr, BufferLength);
...
/*------------------- E2EXf_P07OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P07OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
...
/*------------------- E2EXf_Inv_P07InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P07InPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P07Check(BufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
...
/*------------------- E2EXf_Inv_P07OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P07OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) InputBufferPtr,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P07Check(InputBufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
...
)
{
  ...
  uint32 BufferLength = InputBufferLength;

  /* get config addresses */
  CONSTP2CONST(E2E_P07ConfigType, AUTOMATIC, E2E_CONST) ProfileConfigPtr =
      &(ConfigPtr->P07Config);
  CONSTP2CONST(E2E_SMConfigType, AUTOMATIC, E2E_CONST) SMConfigPtr =
      &(ConfigPtr->SMConfig);
  /* get RAM addresses */
  CONSTP2VAR(E2E_P07CheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr =
      &(CheckStatesPtr->P07CheckState);
  CONSTP2VAR(E2E_SMCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) SMCheckStatePtr =
      &(CheckStatesPtr->SMCheckState);

  RetValCheck = E2E_P07Check(ProfileConfigPtr, CheckStatePtr, BufferPtr, BufferLength);

  /* get profile independent state status */
  RetValMapStatusToSM = E2E_P07MapStatusToSM(RetValCheck, CheckStatePtr->Status);
