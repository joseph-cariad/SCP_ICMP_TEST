...
#include <E2E_P05.h>       /* E2E Profile 5         */
#include <SCrc_CRC16.h>    /* SCrc routine CRC16    */

/*==================[macros]=================================================*/

#if (defined E2EP05_EB_CRC_STARTVALUE) /* to prevent double declaration */
#error E2EP05_EB_CRC_STARTVALUE is already defined
#endif /* #if (defined E2EP05_EB_CRC_STARTVALUE) */

/** \brief Start value for Crc calculation */
#define E2EP05_EB_CRC_STARTVALUE 0xFFFFU

#if (defined E2EP05_EB_COUNTER_MAXVALUE) /* to prevent double declaration */
#error E2EP05_EB_COUNTER_MAXVALUE is already defined
#endif /* #if (defined E2EP05_EB_COUNTER_MAXVALUE) */

/** \brief Maximum value for state counter */
#define E2EP05_EB_COUNTER_MAXVALUE 0xFFU

#if (defined E2EP05_EB_E2EHEADER_CRC_OFFSET) /* to prevent double declaration */
#error E2EP05_EB_E2EHEADER_CRC_OFFSET is already defined
#endif /* #if (defined E2EP05_EB_E2EHEADER_CRC_OFFSET) */

/** \brief Offset in bytes within E2E header where least significant byte of CRC is coded */
#define E2EP05_EB_E2EHEADER_CRC_OFFSET 0U

#if (defined E2EP05_EB_E2EHEADER_CRC_SIZE) /* to prevent double declaration */
#error E2EP05_EB_E2EHEADER_CRC_SIZE is already defined
#endif /* #if (defined E2EP05_EB_E2EHEADER_CRC_SIZE) */

/** \brief Crc size in bytes within E2E header. */
#define E2EP05_EB_E2EHEADER_CRC_SIZE 2U

#if (defined E2EP05_EB_E2EHEADER_COUNTER_OFFSET) /* to prevent double declaration */
#error E2EP05_EB_E2EHEADER_COUNTER_OFFSET is already defined
#endif /* #if (defined E2EP05_EB_E2EHEADER_COUNTER_OFFSET) */

/** \brief Offset in bytes within E2E header where least significant byte of Counter is coded */
#define E2EP05_EB_E2EHEADER_COUNTER_OFFSET 2U

#if (defined E2EP05_EB_E2EHEADER_SIZE_BITS) /* to prevent double declaration */
#error E2EP05_EB_E2EHEADER_SIZE_BITS is already defined
#endif /* #if (defined E2EP05_EB_E2EHEADER_SIZE_BITS) */

/** \brief E2E Profile 05 header size in bits. */
#define E2EP05_EB_E2EHEADER_SIZE_BITS 24U

#if (defined E2EP05_EB_WRITE2BYTE_VALUE) /* to prevent double declaration */
#error E2EP05_EB_WRITE2BYTE_VALUE is already defined
#endif /* #if (defined E2EP05_EB_WRITE2BYTE_VALUE) */

/** \brief Write a 2-byte value in Little Endian order to the passed Data Pointer */
...
#if (defined E2EP05_EB_READ2BYTE_VALUE) /* to prevent double declaration */
#error E2EP05_EB_READ2BYTE_VALUE is already defined
#endif /* #if (defined E2EP05_EB_READ2BYTE_VALUE) */

/** \brief Read a 2-byte value in Little Endian order from the passed Data Pointer */
 ...
 */
static FUNC(Std_ReturnType, E2E_CODE) E2E_EB_P05ProtectNullPtrChecks
(
 ...
 * \retval E_NOT_OK At least on input parameter is invalid
 */
static FUNC(Std_ReturnType, E2E_CODE) E2E_EB_P05ConfigParamChecks
(
    P2CONST(E2E_P05ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr
);
 ...
 * \retval E_NOT_OK At least on input parameter is invalid
 */
static FUNC(Std_ReturnType, E2E_CODE) E2E_EB_P05CheckNullPtrChecks
(
    P2CONST(E2E_P05ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
...
/**
 * \brief Protects the array/buffer to be transmitted using the E2E Profile P05.
 *
 ...
 */
static FUNC(Std_ReturnType, E2E_CODE) E2E_EB_P05Protect
(
...
/**
 * \brief Check the received Data using the E2E Profile P05.
 *
 ...
 * \pre All input parameters are valid
 *
 * \return Profile 5 communication status value of the passed data
 */
static FUNC(E2E_P05CheckStatusType, E2E_CODE) E2E_EB_P05Check
(
...
)
{
     ...
     * Note: Since State contains only a Counter where all values are valid, noch
     *       check of State parameters is required. */
    if (E2E_EB_P05ProtectNullPtrChecks(ConfigPtr, StatePtr, DataPtr) != E_OK)
    {
        RetVal = E2E_E_INPUTERR_NULL;
    }
    else if (E2E_EB_P05ConfigParamChecks(ConfigPtr) != E_OK)
    {
    ...
    else
    {
        ...
        RetVal = E2E_EB_P05Protect(
                &(StatePtr->Counter),
                DataPtr,
                (ConfigPtr->Offset / 8U),
                ConfigPtr->DataID,
                (ConfigPtr->DataLength / 8U));
    }
...
)
{
    ...
    if (E2E_EB_P05CheckNullPtrChecks(ConfigPtr, StatePtr, DataPtr, Length) != E_OK)
    {
        RetVal = E2E_E_INPUTERR_NULL;
    }
    else if (E2E_EB_P05ConfigParamChecks(ConfigPtr) != E_OK)
    ...
    else
    {
        ...
        const E2E_P05CheckStatusType E2EP05_EB_Status = E2E_EB_P05Check(
                &(StatePtr->Counter),
                DataPtr,
                (ConfigPtr->Offset / 8U),
                ConfigPtr->DataID,
...
)
{
    ...
    if (NULL_PTR != StatePtr)
    {
        ...
        StatePtr->Counter = E2EP05_EB_COUNTER_MAXVALUE;
        StatePtr->Status = E2E_P05STATUS_ERROR;
        RetVal = E2E_E_OK;
    }
    else
...
)
{
    ...
    if (CheckReturn == E2E_E_OK)
    {
        switch (Status)
        {
        case E2E_P05STATUS_OK: /* fall through */
        case E2E_P05STATUS_OKSOMELOST:
            RetVal = E2E_P_OK;
            break;
...
)
{
    ...
    /* sanity and input checks: E2E_P05ConfigType */
    ...
    else if (E2EP05_EB_E2EHEADER_SIZE_BITS > ConfigPtr->DataLength)
    {
       RetVal = E2E_E_INPUTERR_WRONG;
    }
    else if ((uint16)(ConfigPtr->DataLength - E2EP05_EB_E2EHEADER_SIZE_BITS) < ConfigPtr->Offset)
    {
       RetVal = E2E_E_INPUTERR_WRONG;
    }
    else
...
)
{
    ...
    if (0U < CrcByteOffset)
    {
       Crc = SCrc_CalculateCRC16(&DataPtr[0], CrcByteOffset, E2EP05_EB_CRC_STARTVALUE, TRUE);
       Crc = SCrc_CalculateCRC16
             (
                 &DataPtr[CrcByteOffset + E2EP05_EB_E2EHEADER_CRC_SIZE],
                 ((uint32) Length - CrcByteOffset) - E2EP05_EB_E2EHEADER_CRC_SIZE,
                 Crc,
                 FALSE
             );
    }
    else
    {
      Crc = SCrc_CalculateCRC16
            (
                &DataPtr[E2EP05_EB_E2EHEADER_CRC_SIZE],
                (uint32) Length - E2EP05_EB_E2EHEADER_CRC_SIZE,
                E2EP05_EB_CRC_STARTVALUE,
                TRUE
            );
    }
...
)
{
    ...
    /* 2) Write Counter in Big Endian order */
    DataPtr[(uint16)(Offset + E2EP05_EB_E2EHEADER_COUNTER_OFFSET)] = Counter;

    {
        const uint16 CrcByteOffset = Offset + E2EP05_EB_E2EHEADER_CRC_OFFSET;

        /* 3) Compute CRC */
        const uint16 ComputedCRC = E2E_EB_CalculateCrc(DataPtr, CrcByteOffset, Length, DataID);

        /* 4) Write CRC */
        E2EP05_EB_WRITE2BYTE_VALUE(&DataPtr[CrcByteOffset], ComputedCRC);
    }
...
{
    E2E_P05CheckStatusType RetVal = E2E_P05STATUS_ERROR;
    ...
    if (TRUE == NewDataAvailable)
    {
        ...
        const uint8 ReceivedCounter = DataPtr[Offset+E2EP05_EB_E2EHEADER_COUNTER_OFFSET];

        /* Read CRC */
        const uint16 ReceivedCRC = E2EP05_EB_READ2BYTE_VALUE(&DataPtr[CrcByteOffset]);
        ...
        else
        {
            ...
            const uint8 DeltaCounter = (ReceivedCounter >= LastStateCounter) ?
                  ...
                  (uint8)((uint8)(E2EP05_EB_COUNTER_MAXVALUE - LastStateCounter)
                        +  ReceivedCounter + 1U);
            ...
            if (DeltaCounter > MaxDeltaCounter)
            {
                ...
                RetVal = E2E_P05STATUS_WRONGSEQUENCE;
            }
    ...
    else
    {
        ...
        RetVal = E2E_P05STATUS_NONEWDATA;
    }
