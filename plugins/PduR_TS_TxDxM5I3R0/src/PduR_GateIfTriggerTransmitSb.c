...
FUNC(Std_ReturnType, PDUR_CODE) PduR_GateIfTriggerTransmitSb
(
    PduIdType GPduId,
...
)
{
   ...
   DBG_PDUR_GATEIFTRIGGERTRANSMITSB_ENTRY(GPduId, PduInfoPtr);
...
#endif /* #if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      PduInfoPtr->SduLength = pBufSbConfig[BufferId].Length;
       ...
       * lower layer module */
      TS_MemCpy(PduInfoPtr->SduDataPtr, pBufMem, pBufSbConfig[BufferId].Length);
   ...
   DBG_PDUR_GATEIFTRIGGERTRANSMITSB_EXIT(RetVal, GPduId, PduInfoPtr);
...
typedef void PduR_PreventEmptyTranslationUnit_GateIfTriggerTransmitSbType;
