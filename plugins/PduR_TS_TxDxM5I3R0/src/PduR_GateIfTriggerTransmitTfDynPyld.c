...
FUNC(Std_ReturnType, PDUR_CODE) PduR_GateIfTriggerTransmitTfDynPyld
(
    PduIdType GPduId,
...
)
{
   ...
   DBG_PDUR_GATEIFTRIGGERTRANSMITTFDYNPYLD_ENTRY(GPduId, PduInfoPtr);
...
#endif /* #if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      PduInfoPtr->SduLength = pUsedSduLength[HeadPos];

      /* Copy data from T-FIFO buffer of PDU Router to memory location provided by
       * lower layer module */
      TS_MemCpy(PduInfoPtr->SduDataPtr, &pBufMem[HeadPos * LenTfBuffer], pUsedSduLength[HeadPos]);
   ...
   DBG_PDUR_GATEIFTRIGGERTRANSMITTFDYNPYLD_EXIT(RetVal, GPduId, PduInfoPtr);
...
typedef void PduR_PreventEmptyTranslationUnit_GateIfTriggerTransmitTfDynPyldType;
