...
FUNC(void, PDUR_CODE) PduR_GateIfRxIndicationDf
(
   PduIdType GPduId,
   P2VAR(PduInfoType, AUTOMATIC, PDUR_APPL_DATA) PduInfoPtr
)
{
   DBG_PDUR_GATEIFRXINDICATIONDF_ENTRY(GPduId, PduInfoPtr);
...
#endif /* #if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      /* pointer to D-FIFO buffer information in RAM for static payload */
      ...
      if (pBufDfInfo[BufferId].ConfP > 0U)
      {
         ...
         CONSTP2CONST(PduR_BufDfConfigType, AUTOMATIC, PDUR_APPL_CONST) pBufDfConfig =
            PDUR_GET_CONFIG_ADDR(PduR_BufDfConfigType,
            PduR_GConfigPtr->PduR_BufDfConfigRef);
   ...
   DBG_PDUR_GATEIFRXINDICATIONDF_EXIT(GPduId, PduInfoPtr);
}
