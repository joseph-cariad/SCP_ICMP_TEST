...
#if ((PDUR_UPTP_CHANGEPARAM_ENABLED == STD_ON) && (PDUR_ZERO_COST_OPERATION_TP == STD_OFF))
...
FUNC(Std_ReturnType, PDUR_CODE) PduR_GenericTpChangeParameter
(
    PduIdType RxPduId,
...
)
{
   ...
   DBG_PDUR_GENERICTPCHANGEPARAMETER_ENTRY(RxPduId, TPParameter, TPParameterValue,
                                           RTabUpTpConfigIdx);
   ...
   if(PduR_State == PDUR_UNINIT)
   {
      ...
      PDUR_DET_REPORT_ERROR(PDUR_SID_UP_CHANGEPARAREQ, PDUR_E_INVALID_REQUEST);
   }
   /* range check of PDU ID */
   else if (RxPduId >= pRTabUpTpConfig->RTabRxCount)
   {
      PDUR_DET_REPORT_ERROR(PDUR_SID_UP_CHANGEPARAREQ, PDUR_E_PDU_ID_INVALID);
   ...
   else
   {
...
#endif /* (PDUR_ROUTINGPATHGROUPS_SUPPORT == STD_ON) */
      {
         ...
         /* pointer to ChangeParameter function */
         const PduR_ChangeParameterFpType ChangeParameterFp =
            PduR_TpModuleAPI[TargetAPIRef].TpChangeParameter;

         /* check if target function pointer is different to null pointer,
            i.e. ChangeParameter functionality is enabled for lower layer TP module. */
         if(NULL_PTR != ChangeParameterFp)
         {
            /* get target PDU ID */
            const PduR_PduIdType TargetPduId = pTpRTabUpRx[RxPduId].TargetPduId;

            /* call ChangeParameter function */
            RetVal = ChangeParameterFp(TargetPduId, TPParameter, TPParameterValue);
         }
      }
   }


   DBG_PDUR_GENERICTPCHANGEPARAMETER_EXIT(RetVal, RxPduId, TPParameter, TPParameterValue,
                                          RTabUpTpConfigIdx);
   return RetVal;
}
...
#else /* if (PDUR_UPTP_CHANGEPARAM_ENABLED == STD_ON) && .. */
...
#endif /* if (PDUR_UPTP_CHANGEPARAM_ENABLED == STD_ON) && .. */
/*==================[end of file]===========================================*/
