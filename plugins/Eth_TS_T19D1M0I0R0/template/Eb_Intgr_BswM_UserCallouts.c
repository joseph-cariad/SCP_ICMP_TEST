...
/* There are several Eth controllers which requires a clock from the EthTrcv during the
* initialization (call of Eth_ControllerInit() because during initialization a reset of the Eth
* controller is done.
* Example of such Eth controllers are: TC29XT, TC38XQ, SPC58XC, SPC58XG,  STA1385M3
* Depending on the HW and the EthTrcv the EthTrcv must be activated to provide a clock to the
* Eth controller (via Eth_WriteMii() calls).
* To solve this issue a own intialization function for the MII mng interface is introduced:
* Eth_MiiInit()
* This initialization function does not reset the Eth controller, it only initialized the MII
* managment interface so that Eth_WriteMii() and Eth_ReadMii() can be used.
* Thus with help of these function the EthTrcv can be activated so that the EthTrcv provides the
* needed clocks to the Eth controller (for a Eth controller reset).
* After activation of the EthTrcv and usual Eth_ControllerInit() function must be called and
* the proceeding is as usual.
*
* Following configuration parameters must be set in the Eth module:
* - EthSupportAPI: ASR430b
* Following configuration parameters must be set in the EthIf module:
* - EthIfInitControllersTransceivers: false
* - EthIfAsyncEthCtrlModeSupport: true
*/

#if 1
/* Reset and initialization of the Eth controller is possble without activating the EthTrcv before */
#if(BASE_ETH_ENABLED==STD_ON)
 #if defined(ETH_EB_CONTROLLERINIT)
  ETH_EB_CONTROLLERINIT();
#endif
#endif
#if(BASE_ETHTRCV_ENABLED==STD_ON)
 #if defined(ETHTRCV_EB_TRANSEIVERINIT)
  ETHTRCV_EB_TRANSEIVERINIT();
#endif
#endif

#else
/* Before reset and initialization of the Eth controller the EthTrcv must be activated via
*  Eth_WriteMii().
*/
#if(BASE_ETH_ENABLED==STD_ON)
 #if defined(ETH_EB_CONTROLLERINIT)
...
#endif
#endif
/* Add the activation of the EthTrcv via Eth_WriteMii()/Eth_ReadMii() */
#if(BASE_ETH_ENABLED==STD_ON)
 #if defined(ETH_EB_CONTROLLERINIT)
  ETH_EB_CONTROLLERINIT();
#endif
