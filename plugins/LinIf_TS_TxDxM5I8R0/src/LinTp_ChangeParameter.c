 ...
 * This service is used to request the change of reception parameter STmin for
 * a specified N-SDU.*/
...
)
{
    ...
    DBG_LINTP_CHANGEPARAMETER_ENTRY(id,parameter,value);
    ...
    if(LINTP_UNINIT_ACCESS)
    {
        ...
        (void)Det_ReportError( LINIF_MODULE_ID,
                               LINIF_INSTANCE_ID,
                               LINTP_API_CHANGEPARAMREQEST,
                               LINIF_E_UNINIT
                             );
    }

    /* check for valid N-SDU ID */
    else if( id >= LinTp_ConfigPtr->LinTpNumberOfRxNSdu )
    {
        (void)Det_ReportError( LINIF_MODULE_ID,
                               LINIF_INSTANCE_ID,
                               LINTP_API_CHANGEPARAMREQEST,
    ...
    DBG_LINTP_CHANGEPARAMETER_EXIT(result,id,parameter,value);
