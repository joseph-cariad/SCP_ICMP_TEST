...
#include <E2E_PRN_E2EXf.h>   /* E2EXf APIs for E2E Profile PRN */
...
/* Call PRN dependent check functions */
static FUNC(Std_ReturnType, E2E_CODE) E2EXf_Inv_PRNCheck
(
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
    P2VAR(E2E_PRNCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr,
    P2VAR(E2E_SMCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) SMCheckStatePtr,
    P2CONST(E2E_PRNConfigType, AUTOMATIC, E2E_APPL_CONST) ProfileConfigPtr,
...
/*------------------- E2EXf_PRNInPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_PRNInPlace_Transformer
(
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      ...
      CONSTP2CONST(E2E_PRNConfigType, AUTOMATIC, E2E_CONST) ProfileCfgPtr =
          &(ConfigPtr->PRNConfig);
      ...
      *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE)BufferLength;
      ...
      else
      {
        ...
        RetValProt = E2E_PRNProtect(ProfileCfgPtr, ProtectStatePtr, BufferPtr);
...
/*------------------- E2EXf_PRNOutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_PRNOutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
...
/*------------------- E2EXf_Inv_PRNInPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_PRNInPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
...
)
{
  ...
  {
    /* get config addresses */
    CONSTP2CONST(E2E_PRNConfigType, AUTOMATIC, E2E_CONST) ProfileConfigPtr =
        &(ConfigPtr->PRNConfig);
    ...
    /* get RAM addresses */
    CONSTP2VAR(E2E_PRNCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr =
        &(CheckStatesPtr->PRNCheckState);
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_PRNCheck(BufferPtr, InputBufferLength, CheckStatePtr, SMCheckStatePtr,
...
/*------------------- E2EXf_Inv_PRNOutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_PRNOutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) InputBufferPtr,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      /* Copy InputBuffer to Buffer to re-use E2EXf_Inv_PRNInPlace_Transformer function */
      ...
      RetVal = E2EXf_Inv_PRNInPlace_Transformer(BufferPtr, BufferLengthPtr, InputBufferLength,
...
)
{
  ...
  /* data byte for call of E2E_PRNCheck if no new data are available */
  const uint8 SingleByte = 0U;
  /* initialize data pointer for call of E2E_PRNCheck */
  P2CONST(uint8, AUTOMATIC, E2E_CONST) DataPtr = BufferPtr;

  /* The precondition check of the inverted transformer function excludes buffer pointer is null
   * and input buffer length greater 0. With this precondition check performed before, the check
  ...
  RetValCheck = E2E_PRNCheck(ProfileConfigPtr, CheckStatePtr, DataPtr);
  ...
  RetValMapStatusToSM = E2E_PRNMapStatusToSM(RetValCheck, CheckStatePtr->Status, ProfileBehavior);
