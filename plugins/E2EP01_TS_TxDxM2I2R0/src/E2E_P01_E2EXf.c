...
/* Call P01 dependend check functions */
static FUNC(Std_ReturnType, E2E_CODE) E2EXf_Inv_P01Check
(
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
    P2VAR(E2E_P01CheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr,
    P2VAR(E2E_SMCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) SMCheckStatePtr,
    P2CONST(E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ProfileConfigPtr,
...
/*------------------- E2EXf_P01InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P01InPlace_Transformer
(
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      ...
      CONSTP2CONST(E2E_P01ConfigType, AUTOMATIC, E2E_CONST) ProfileCfgPtr =
          &(ConfigPtr->P01Config);
      ...
      *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE)BufferLength;
      ...
      else
      {
        ...
        RetValProt = E2E_P01Protect(ProfileCfgPtr, ProtectStatePtr, BufferPtr);
...
/*------------------- E2EXf_P01OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P01OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
...
/*------------------- E2EXf_Inv_P01InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P01InPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
...
)
{
  ...
  {
    /* get config addresses */
    CONSTP2CONST(E2E_P01ConfigType, AUTOMATIC, E2E_CONST) ProfileConfigPtr =
        &(ConfigPtr->P01Config);
    ...
    /* get RAM addresses */
    CONSTP2VAR(E2E_P01CheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr =
        &(CheckStatesPtr->P01CheckState);
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P01Check(BufferPtr, InputBufferLength, CheckStatePtr, SMCheckStatePtr,
...
/*------------------- E2EXf_Inv_P01OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P01OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) InputBufferPtr,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      /* Copy InputBuffer to Buffer to re-use E2EXf_Inv_P01InPlace_Transformer function */
      ...
      RetVal = E2EXf_Inv_P01InPlace_Transformer(BufferPtr, BufferLengthPtr, InputBufferLength,
...
)
{
  ...
  /* data byte for call of E2E_P01Check if no new data are available */
  const uint8 SingleByte = 0U;
  /* initialize data pointer for call of E2E_P01Check */
  ...
  RetValCheck = E2E_P01Check(ProfileConfigPtr, CheckStatePtr, DataPtr);
  ...
  RetValMapStatusToSM = E2E_P01MapStatusToSM(RetValCheck, CheckStatePtr->Status, ProfileBehavior);
