...
define build-exe-file
	...
	&& $(LINK) --via=$(LNK_INC) --scatter=$(GET_LOC_FILE) -o $(HEXFILE)\
	&& echo === BUILD DONE ===
endef
...
define build-c-file
	...
	$(EB_VERBOSE)if exist $(OBJ_DIR)/$(notdir $(basename $@)).d $(MV) $(OBJ_DIR)/$(notdir $(basename $@)).d $(DEPEND_DIR)/$(notdir $(basename $@)).d
...
# $(ASM_2)...............: executable for specific source file from Resource module 
# $(ASM_2_OPT)...........: older assembler type options
#---------------------------------------------------------------------------------------------------
# Desciption............: defines the call to the compiler
####################################################################################################
define build-s-file
	...
	$(if $(findstring asm, $(suffix $<)), $(EB_VERBOSE)$(ASM) @$(ASM_INC) -c -x assembler-with-cpp  $< -o $(OBJ_DIR)/$(notdir $(basename $@)).o,\
	$(EB_VERBOSE)$(ASM_2) $(ASM_2_OPT) $< -o $(OBJ_DIR)/$(notdir $(basename $@)).o)
endef
