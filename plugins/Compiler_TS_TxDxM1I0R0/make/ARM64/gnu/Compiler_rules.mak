...
define build-exe-file
	...
	&& $(OBJCPY) -O binary $(subst \,/,$@) $(subst \,/,$(addsuffix .bin, $(basename $@)))\
...
define build-c-file
	...
	$(EB_VERBOSE)$(CC) @$(MAKE_INC) -c -x c $< -o $(OBJ_DIR)/$(notdir $(basename $<).o)
endef
...
define build-link-inc-file-flat
	...
	$(file >$(subst \,/,$@), $(subst /,\,$(LINK_OPT)))
	$(file >>$(subst \,/,$@), -Wl,--start-group)
	$(foreach i,$(LINK_ONLY_OBJ), $(file >>$@,obj/$(notdir $i)))
	$(foreach i,$(LIBRARIES_LINK_ONLY), $(file >>$@,lib/$(notdir $i)))
	$(foreach i,$(OBJECTS_LINK_ONLY), $(file >>$@,lib/$(notdir $i)))
	$(file >>$@,$(addprefix obj/,$(notdir $(FLAT_OUT_LIB))))
	$(file >>$@,$(addprefix obj/,$(notdir $(basename $(PROJECT_BINARY))).a))
	$(file >>$(subst \,/,$@), -Wl,--end-group)
endef
