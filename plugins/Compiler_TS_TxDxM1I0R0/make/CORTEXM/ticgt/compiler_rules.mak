...
define build-c-file
	...
	&& if exist $(notdir $(basename $@)).asm $(MV) $(notdir $(basename $@)).asm $(OBJ_DIR)/$(notdir $(basename $@)).asm
endef
...
# we can not check if build-link-inc-file is defined without expanding it  <- NOT TRUE!!!
