...
define build-exe-file
	$(info Linking $(notdir $@))
	$(EB_VERBOSE)$(LINK) --map $(MAPFILE) -f $(subst \,/,$(LNK_INC)) --config $(GET_LOC_FILE) --output $(subst \,/,$@)\
	&& echo === BUILD DONE ===
endef
...
define build-c-file
	...
	$(EB_VERBOSE)$(CC) -f $(MAKE_INC) $< --output $(OBJ_DIR)/$(notdir $(basename $<).o)
endef
...
define build-s-file
	...
	$(EB_VERBOSE)$(GCC) -MT $(OBJ_DIR)/$(notdir $(basename $<).o) -MM @$(DEPEND_INC) $< -MF $(DEPEND_DIR)/$(notdir $(basename $@)).d
	$(EB_VERBOSE)$(GCC) @$(DEPEND_INC) -E -x c $< -P -o $(OBJ_DIR)/$(notdir $(basename $<).s)
	$(EB_VERBOSE)$(ASM) -f $(ASM_INC) $(OBJ_DIR)/$(notdir $(basename $<).s) -o $(OBJ_DIR)/$(notdir $(basename $<).o)
endef
...
define build-lib-file
	$(CCLIB) $(AR_OPT) --create $@ $(GET_LIB_BUILD_OPTIONS) $<
	$(call SC_CMD_LISTPERFORM,SC_CMD_AR_HELPER,\
	$(CCLIB) $(AR_OPT) -r $@ $(GET_LIB_BUILD_OPTIONS),4,5, $+)
endef
