...
define build-exe-file
	$(EB_VERBOSE)$(LINK) -T $(GET_LOC_FILE) @$(LNK_INC) -o $@\
	&& $(HEX) $(PROJECT_BINARY) -o $(HEXFILE)\
	&& echo === BUILD DONE ===
endef
...
define build-c-file
	...
	&& $(MV) $(OBJ_DIR)/$(notdir $(basename $@)).d $(DEPEND_DIR)/$(notdir $(basename $@)).d\
	&& if exist $(notdir $(basename $@)).s $(MV) $(notdir $(basename $@)).s $(OBJ_DIR)/$(notdir $(basename $@)).s
endef
...
define build-s-file
	...
	$(EB_VERBOSE)$(CC) $(basename $@).$(ASM_FILE_SUFFIX) @$(ASM_INC) -o $(OBJ_DIR)/$(notdir $(basename $@)).o
endef
...
define build-lib-file
	$(call SC_CMD_LISTPERFORM,SC_CMD_AR_HELPER,\
		...
		$(EB_VERBOSE)$(CCLIB) -archive -o $@,20,21, $+)
endef
