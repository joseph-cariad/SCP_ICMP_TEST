...
<stylesheet version="2.0" xmlns="http://www.w3.org/1999/XSL/Transform" xmlns:my="http://www.elektrobit.com/2013/xgen/local" xmlns:x="http://www.elektrobit.com/2013/xgen" xmlns:xml="http://www.w3.org/XML/1998/namespace" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	<template match="x:xgen" mode="inclusions">
		...
		<call-template name="sysinclude">
			<with-param name="filename" select="fn:concat($my:module, '_SymbolicNames_PBcfg.h')" />
		</call-template>
	...
	<template match="x:instance/x:parameter[@name='postbuild-configuration-name']" mode="macros">
		<variable name="instance" select=".." as="element()" />
		<variable name="name" select="my:get-property-value(.)" as="element()?"/>

		<if test="exists($name)">
			<call-template name="macro">
				<with-param name="name" select="$name/text()" />
				<with-param name="value" select="concat(
                    '(',
                    $instance/@name,
                    '.',
                    //x:type[@name = my:resolve-type-references($instance/@type)]/x:struct/x:member[1]/@name,
                    ')'
                    )" />
			</call-template>
		</if>
