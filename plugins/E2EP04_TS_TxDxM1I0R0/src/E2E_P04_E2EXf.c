...
#include <E2E_P04_E2EXf.h>  /* E2EXf APIs for E2E Profile P04 */
...
/* Call P04 dependend check functions */
static FUNC(Std_ReturnType, E2E_CODE) E2EXf_Inv_P04Check
(
...
/*------------------- E2EXf_P04InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P04InPlace_Transformer
(
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      ...
      CONSTP2CONST(E2E_P04ConfigType, AUTOMATIC, E2E_CONST) ProfileCfgPtr =
          &(ConfigPtr->P04Config);
      ...
      *BufferLengthPtr = (E2EXF_XFRMBUFFERLENGTH_TYPE) BufferLength;
      ...
      RetValProt = E2E_P04Protect(ProfileCfgPtr, ProtectStatePtr, BufferPtr, BufferLength);
...
/*------------------- E2EXf_P04OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_P04OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
...
/*------------------- E2EXf_Inv_P04InPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P04InPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    E2EXF_XFRMBUFFERLENGTH_TYPE InputBufferLength,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P04Check(BufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
...
/*------------------- E2EXf_Inv_P04OutOfPlace_Transformer -------------------------*/
FUNC(uint8, E2E_CODE) E2EXf_Inv_P04OutOfPlace_Transformer
(
    P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) BufferPtr,
    P2VAR(E2EXF_XFRMBUFFERLENGTH_TYPE, AUTOMATIC, E2E_APPL_DATA) BufferLengthPtr,
    P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) InputBufferPtr,
...
)
{
  ...
  if(E2EXf_PartitionInitState == E2EXF_ONLINE)
  {
    ...
    if(E_OK == RetVal)
    {
      RetVal = E2EXf_Inv_P04Check(InputBufferPtr, InputBufferLength, CheckStatesPtr, ConfigPtr);
...
)
{
  ...
  /* get config addresses */
  CONSTP2CONST(E2E_P04ConfigType, AUTOMATIC, E2E_CONST) ProfileConfigPtr =
      &(ConfigPtr->P04Config);
  CONSTP2CONST(E2E_SMConfigType, AUTOMATIC, E2E_CONST) SMConfigPtr =
      &(ConfigPtr->SMConfig);
  /* get RAM addresses */
  CONSTP2VAR(E2E_P04CheckStateType, AUTOMATIC, E2E_VAR_CLEARED) CheckStatePtr =
      &(CheckStatesPtr->P04CheckState);
  CONSTP2VAR(E2E_SMCheckStateType, AUTOMATIC, E2E_VAR_CLEARED) SMCheckStatePtr =
      &(CheckStatesPtr->SMCheckState);

  RetValCheck = E2E_P04Check(ProfileConfigPtr, CheckStatePtr, BufferPtr, BufferLength);

  /* get profile independent state status */
  RetValMapStatusToSM = E2E_P04MapStatusToSM(RetValCheck, CheckStatePtr->Status);
