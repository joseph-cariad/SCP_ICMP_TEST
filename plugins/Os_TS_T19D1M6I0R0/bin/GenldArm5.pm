...
# factory interface
#
# - implement all needed functions to satify the provided factory interface
# - the functions actually save all provided data at an internal array for a
#   later use
# #############################################################################
sub _new
{
...
{
	$self = shift;
	my ($symbol, $value, $global, $attrFIXED) = @_;
	...
	if(defined $global) {
		...
		$value =~ s/"//g;
		push(@{$self->{'symbols'}}, "$symbol $value EMPTY $fixed 0 { }\n");
	} else {
		...
		# set a fixed alignment of 16 for extra created "symbols" to ensure that
		# the created load sections which use this "symbols" as start address are
		# also 16 align
		$self->_addSymbol($symbol, '16', $value, $attrFIXED);
	}
}
...
# needed to merge separated function calls to the same memory section.
# ####
	...
	if(not defined $self->{'currentBlock'}) {
		...
		if(defined $name) {
			...
			foreach my $item (@{$self->{'content'}}) {
				if(defined $item->{'name'} && "$item->{'name'}" eq "$name") {
					$found = 1;
					# copy input from new memory section to already exist one
					foreach (@{$self->{'currentBlock'}{'input'}}) {
						push(@{$item->{'input'}}, $_);
					}
			...
			if($found == 0) {
				$self->{'currentBlock'}{'name'} = $name if defined $name;
			} else {
				pop (@{$self->{'content'}});
			}
		}
...
sub _addSymbol
{
	...
	$alignment = defined $align ? "ALIGN $align" : '';
	$addr = defined $value ? $value : '+0';
	$fixed = defined $attrFIXED ? $attrFIXED : '';
...
sub _addOutputSection
{
	...
	die "Parameter preciseFiles not yet implemented for the ARM 5 Toolchain!"
	...
	my $attrUNINIT = (defined $ot->{_arm5_attr_uninit}) ? $ot->{_arm5_attr_uninit} : '';
	my $attrFIXED = (defined $ot->{_arm5_attr_fixed}) ? $ot->{_arm5_attr_fixed} : '';

	# Override region alignment from architecture definition
	defined $startAlign && ($startAlign = $self->_val_to_dec($startAlign));
	# Override end alignment from architecture defintion
	defined $endAlign && ($endAlign = $self->_val_to_dec($endAlign));
    ...
    # the minimum boundary.
   	my $start_align = (defined $startAlign) ? $startAlign : $ot->{align};
	if ($start_align == 0) {
		$start_align = 16;
	}

	# add init and start symbols if provided
	defined $initSymbol && $self->_addSymbol($initSymbol, $start_align, undef, $attrFIXED);
	defined $startSymbol && $self->_addSymbol($startSymbol, $start_align, undef, $attrFIXED);

	$self->_addString("$osecName +0 ALIGN $start_align $attrUNINIT $attrFIXED");
	$self->_addString("{");
	...
	foreach my $file (@$files) {
		...
		if (ref $file eq 'HASH') {
			warn "ARM 5 does not support the selection of archive members $file->{lib}($file->{file})\n";
			$modsel = $file->{file};
		} else {
			$modsel = $file;
		}
	...
	my $end_align = (defined $endAlign) ? $endAlign : $ot->{endalign};
	if (defined $end_align and $end_align == 0) {
		$end_align = 16;
	}

	defined $endSymbol && $self->_addSymbol($endSymbol, undef, undef, $attrFIXED);
	defined $initEndSymbol && $self->_addSymbol($initEndSymbol, $end_align, undef, $attrFIXED);
	$self->_addString(" ");
}
