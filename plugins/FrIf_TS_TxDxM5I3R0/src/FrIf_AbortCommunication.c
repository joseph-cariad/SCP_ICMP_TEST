...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_AbortCommunication
    (
        uint8 FrIf_CtrlIdx
    )
{
    ...
    DBG_FRIF_ABORTCOMMUNICATION_ENTRY(FrIf_CtrlIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ABORTCOMMUNICATION_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ABORTCOMMUNICATION_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg1(FRIF_FR_INSTANCE_IDX,AbortCommunication,FRIF_FR_CTRLIDX);
    }

    /* return result */

    DBG_FRIF_ABORTCOMMUNICATION_EXIT(RetValue,FrIf_CtrlIdx);
    return RetValue;
}
