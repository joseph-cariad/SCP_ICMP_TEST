 ...
 * \param FrIf_SyncStatePtr (out)     Address the synchronization state is written to.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetSyncState(
    uint8 FrIf_CtrlIdx,
    P2VAR(Fr_SyncStateType, AUTOMATIC, FRIF_APPL_DATA) FrIf_SyncStatePtr)
{
    Std_ReturnType RetValue = E_NOT_OK;

    DBG_FRIF_GETSYNCSTATE_ENTRY(FrIf_CtrlIdx,FrIf_SyncStatePtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETSYNCSTATE_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETSYNCSTATE_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,GetSyncState,FRIF_FR_CTRLIDX, FrIf_SyncStatePtr);
    }

    /* return result */

    DBG_FRIF_GETSYNCSTATE_EXIT(RetValue,FrIf_CtrlIdx,FrIf_SyncStatePtr);
    return RetValue;
}
...
#endif /* FRIF_GETSYNCSTATE_API_ENABLE */
