 ...
 * \param FrIf_Cycle (in)         Communication Cycle the alarm should elapse.
...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_SetAbsoluteTimer
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_SETABSOLUTETIMER_ENTRY(FrIf_CtrlIdx,FrIf_AbsTimerIdx,FrIf_Cycle,FrIf_Offset);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_SETABSOLUTETIMER_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_SETABSOLUTETIMER_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg4(FRIF_FR_INSTANCE_IDX,SetAbsoluteTimer,
                    FRIF_FR_CTRLIDX, FrIf_AbsTimerIdx, FrIf_Cycle, FrIf_Offset);
    }

    /* return result */

    DBG_FRIF_SETABSOLUTETIMER_EXIT(RetValue,FrIf_CtrlIdx,FrIf_AbsTimerIdx,FrIf_Cycle,FrIf_Offset);
    return RetValue;
}
