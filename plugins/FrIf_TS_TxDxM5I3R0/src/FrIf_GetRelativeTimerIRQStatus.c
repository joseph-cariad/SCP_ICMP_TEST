...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetRelativeTimerIRQStatus
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETRELATIVETIMERIRQSTATUS_ENTRY(FrIf_CtrlIdx,FrIf_RelTimerIdx,FrIf_IRQStatusPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETRELATIVETIMERIRQSTATUS_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETRELATIVETIMERIRQSTATUS_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg3(FRIF_FR_INSTANCE_IDX,GetRelativeTimerIRQStatus,FRIF_FR_CTRLIDX, FrIf_RelTimerIdx, FrIf_IRQStatusPtr);
    }

    /* return result */

    DBG_FRIF_GETRELATIVETIMERIRQSTATUS_EXIT(RetValue,FrIf_CtrlIdx,FrIf_RelTimerIdx,FrIf_IRQStatusPtr);
    return RetValue;
}
