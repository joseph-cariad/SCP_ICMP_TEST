...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_AckRelativeTimerIRQ
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_ACKRELATIVETIMERIRQ_ENTRY(FrIf_CtrlIdx,FrIf_RelTimerIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ACKRELATIVETIMERIRQ_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ACKRELATIVETIMERIRQ_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...

    {
      ...
      RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,AckRelativeTimerIRQ,FRIF_FR_CTRLIDX,FrIf_RelTimerIdx);
    }

    /* return result */

    DBG_FRIF_ACKRELATIVETIMERIRQ_EXIT(RetValue,FrIf_CtrlIdx,FrIf_RelTimerIdx);
    return RetValue;
}
