 ...
 * \param FrIf_TrcvWUReasonPtr (out)    Address to write the transceiver WU Reason to.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetTransceiverWUReason
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETTRANSCEIVERWUREASON_ENTRY(FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_TrcvWUReasonPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERWUREASON_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERWUREASON_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERWUREASON_SERVICE_ID, FRIF_E_INV_CHNL_IDX);
    }
...

    {
        ...
        if((FRIF_FRTRCV_INSTANCE_IDX != FRIF_INVALID_ENTRY) &&
           (FrIf_Get_FrTrcvFunctionPtr(FRIF_FRTRCV_INSTANCE_IDX,GetTransceiverWUReason) != NULL_PTR))

#endif /* FRIF_SINGLE_FRTRCV_OPT_ENABLE */
        {
            /* call wrapped FrTrcv service */
            RetValue = FrIf_Call_FrTrcv(FRIF_FRTRCV_INSTANCE_IDX,GetTransceiverWUReason)
                (
                    FRIF_GET_CONFIG_ADDR(FrIf_CtrlType,
                    FrIf_RootConfigPtr->FrIf_CtrlRef)[FRIF_CTRLIDX].Trcv[FrIf_ChnlIdx].TrcvDeviceIdx,
    ...
    DBG_FRIF_GETTRANSCEIVERWUREASON_EXIT(RetValue,FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_TrcvWUReasonPtr);
    return RetValue;
}
