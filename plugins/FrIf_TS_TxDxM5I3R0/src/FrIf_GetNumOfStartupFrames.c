 ...
 * \param FrIf_NumOfStartupFramesPtr (out)   Address where the number of startup frames
 *                                           seen within the last even/odd cycle pair shall be
 *                                           stored.
 *
 * \retval  E_OK                Function serviced successfully.
 * \retval  E_NOT_OK            Function execution failed.
 */
FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetNumOfStartupFrames
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETNUMOFSTARTUPFRAMES_ENTRY(FrIf_CtrlIdx,FrIf_NumOfStartupFramesPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETNUMOFSTARTUPFRAMES_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETNUMOFSTARTUPFRAMES_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,GetNumOfStartupFrames,FRIF_FR_CTRLIDX,FrIf_NumOfStartupFramesPtr);
    }

    /* return result */

    DBG_FRIF_GETNUMOFSTARTUPFRAMES_EXIT(RetValue,FrIf_CtrlIdx,FrIf_NumOfStartupFramesPtr);
    return RetValue;
}
