...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_CancelRelativeTimer
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_CANCELRELATIVETIMER_ENTRY(FrIf_CtrlIdx,FrIf_RelTimerIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CANCELRELATIVETIMER_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CANCELRELATIVETIMER_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,CancelRelativeTimer,FRIF_FR_CTRLIDX, FrIf_RelTimerIdx);
    }

    /* return result */

    DBG_FRIF_CANCELRELATIVETIMER_EXIT(RetValue,FrIf_CtrlIdx,FrIf_RelTimerIdx);
    return RetValue;
}
