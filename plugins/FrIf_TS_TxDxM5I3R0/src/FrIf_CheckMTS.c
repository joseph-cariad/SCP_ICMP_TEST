 ...
 * \param FrIf_ChnlIdx (in)       Channel the MTS should be received on.
 * \param FrIf_MTSStatusPtr (out) Address the MTS status is written to.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_CheckMTS
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_CHECKMTS_ENTRY(FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_MTSStatusPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CHECKMTS_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CHECKMTS_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
      ...
      RetValue = FrIf_Call_Fr_Arg3(FRIF_FR_INSTANCE_IDX,CheckMTS,FRIF_FR_CTRLIDX, FrIf_ChnlIdx, FrIf_MTSStatusPtr);
    }

    /* return result */

    DBG_FRIF_CHECKMTS_EXIT(RetValue,FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_MTSStatusPtr);
    return RetValue;
}
