 ...
 *
 * \note    The function must be called in CC POC-state "ready".
 * \note    Depending on configuration, POC-command ALLOW_COLDSTART is invoked before Startup.
 *
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_StartCommunication
    (
        uint8 FrIf_CtrlIdx
    )
{
    ...
    DBG_FRIF_STARTCOMMUNICATION_ENTRY(FrIf_CtrlIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_STARTCOMMUNICATION_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_STARTCOMMUNICATION_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg1(FRIF_FR_INSTANCE_IDX,StartCommunication,FRIF_FR_CTRLIDX);
    }

    /* return result */

    DBG_FRIF_STARTCOMMUNICATION_EXIT(RetValue,FrIf_CtrlIdx);
    return RetValue;
}
