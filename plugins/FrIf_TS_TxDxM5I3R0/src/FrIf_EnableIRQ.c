...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_EnableIRQ
    (
        uint8 FrIf_CtrlIdx,
        uint8 FrIf_IRQIdx
    )
{
    ...
    DBG_FRIF_ENABLEIRQ_ENTRY(FrIf_CtrlIdx,FrIf_IRQIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ENABLEIRQ_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_ENABLEIRQ_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,EnableIRQ,FRIF_FR_CTRLIDX,FrIf_IRQIdx);
    }

    /* return result */

    DBG_FRIF_ENABLEIRQ_EXIT(RetValue,FrIf_CtrlIdx,FrIf_IRQIdx);
    return RetValue;
}
