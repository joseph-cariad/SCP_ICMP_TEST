...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_ClearTransceiverWakeup
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_CLEARTRANSCEIVERWAKEUP_ENTRY(FrIf_CtrlIdx,FrIf_ChnlIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CLEARTRANSCEIVERWAKEUP_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CLEARTRANSCEIVERWAKEUP_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_CLEARTRANSCEIVERWAKEUP_SERVICE_ID, FRIF_E_INV_CHNL_IDX);
    }
...

    {
        ...
        if((FRIF_FRTRCV_INSTANCE_IDX != FRIF_INVALID_ENTRY) &&
           (FrIf_Get_FrTrcvFunctionPtr(FRIF_FRTRCV_INSTANCE_IDX,ClearTransceiverWakeup) != NULL_PTR))

#endif /* FRIF_SINGLE_FRTRCV_OPT_ENABLE */
        {
            /* call wrapped FrTrcv service */
            RetValue = FrIf_Call_FrTrcv(FRIF_FRTRCV_INSTANCE_IDX,ClearTransceiverWakeup)
                       (
    ...
    DBG_FRIF_CLEARTRANSCEIVERWAKEUP_EXIT(RetValue,FrIf_CtrlIdx,FrIf_ChnlIdx);
    return RetValue;
}
