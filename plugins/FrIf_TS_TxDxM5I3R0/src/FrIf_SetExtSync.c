 ...
 * \param FrIf_Offset (in)            Offset correction application mode.
 * \param FrIf_Rate (in)              Rate correction application mode.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_SetExtSync
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_SETEXTSYNC_ENTRY(FrIf_CtrlIdx,FrIf_Offset,FrIf_Rate);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_SETEXTSYNC_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_SETEXTSYNC_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg3(FRIF_FR_INSTANCE_IDX,SetExtSync,
                   FRIF_FR_CTRLIDX, FrIf_Offset, FrIf_Rate);
    }

    /* return result */

    DBG_FRIF_SETEXTSYNC_EXIT(RetValue,FrIf_CtrlIdx,FrIf_Offset,FrIf_Rate);
    return RetValue;
}
