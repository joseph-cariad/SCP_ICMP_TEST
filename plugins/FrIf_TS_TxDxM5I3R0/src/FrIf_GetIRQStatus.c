...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetIRQStatus
    (
        uint8 FrIf_CtrlIdx,
        uint8 FrIf_IRQIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETIRQSTATUS_ENTRY(FrIf_CtrlIdx,FrIf_IRQIdx,FrIf_IRQStatusPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETIRQSTATUS_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETIRQSTATUS_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg3(FRIF_FR_INSTANCE_IDX,GetIRQStatus,FRIF_FR_CTRLIDX,FrIf_IRQIdx,FrIf_IRQStatusPtr);
    }

    /* return result */

    DBG_FRIF_GETIRQSTATUS_EXIT(RetValue,FrIf_CtrlIdx,FrIf_IRQIdx,FrIf_IRQStatusPtr);
    return RetValue;
}
