 ...
 * \param FrIf_CyclePtr (out)         Address to write the current cycle counter value to.
 * \param FrIf_MacroTickPtr (out)     Address to write the current macrotick counter value to.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetGlobalTime
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETGLOBALTIME_ENTRY(FrIf_CtrlIdx,FrIf_CyclePtr,FrIf_MacroTickPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETGLOBALTIME_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETGLOBALTIME_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg3(FRIF_FR_INSTANCE_IDX,GetGlobalTime,FRIF_FR_CTRLIDX, FrIf_CyclePtr, FrIf_MacroTickPtr);
    }

    /* return result */

    DBG_FRIF_GETGLOBALTIME_EXIT(RetValue,FrIf_CtrlIdx,FrIf_CyclePtr,FrIf_MacroTickPtr);
    return RetValue;
}
