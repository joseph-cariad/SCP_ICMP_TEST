...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetTransceiverError
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETTRANSCEIVERERROR_ENTRY(FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_BranchIdx,FrIf_BusErrorState);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERERROR_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERERROR_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETTRANSCEIVERERROR_SERVICE_ID, FRIF_E_INV_CHNL_IDX);
    }
...

    {
        ...
        if((FRIF_FRTRCV_INSTANCE_IDX != FRIF_INVALID_ENTRY) &&
           (FrIf_Get_FrTrcvFunctionPtr(FRIF_FRTRCV_INSTANCE_IDX,GetTransceiverError) != NULL_PTR))

#endif /* FRIF_SINGLE_FRTRCV_OPT_ENABLE */
        {
            /* call wrapped FrTrcv service */
            RetValue = FrIf_Call_FrTrcv(FRIF_FRTRCV_INSTANCE_IDX,GetTransceiverError)
                    (
                     FRIF_GET_CONFIG_ADDR(FrIf_CtrlType,
                     FrIf_RootConfigPtr->FrIf_CtrlRef)[FRIF_CTRLIDX].Trcv[FrIf_ChnlIdx].TrcvDeviceIdx,
                     FrIf_BranchIdx,
    ...
    DBG_FRIF_GETTRANSCEIVERERROR_EXIT(RetValue,FrIf_CtrlIdx,FrIf_ChnlIdx,FrIf_BranchIdx,FrIf_BusErrorState);
    return RetValue;
}
