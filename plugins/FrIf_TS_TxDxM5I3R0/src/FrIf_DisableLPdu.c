...
FUNC(Std_ReturnType,FRIF_CODE) FrIf_DisableLPdu
    (
        uint8 FrIf_CtrlIdx,
        uint16 FrIf_LPduIdx
    )
{
    ...
    DBG_FRIF_DISABLELPDU_ENTRY(FrIf_CtrlIdx,FrIf_LPduIdx);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_DISABLELPDU_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_DISABLELPDU_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,DisableLPdu,FRIF_FR_CTRLIDX,FrIf_LPduIdx);
    }

    /* return result */

    DBG_FRIF_DISABLELPDU_EXIT(RetValue,FrIf_CtrlIdx,FrIf_LPduIdx);
    return RetValue;
}
