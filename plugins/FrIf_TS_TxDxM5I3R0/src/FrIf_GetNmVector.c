 ...
 * \param FrIf_NmVectorPtr (out)      Address to write the Nm-Vector to.
 *
 * \retval  E_OK               Function serviced successfully.
 * \retval  E_NOT_OK           Function execution failed.
 */

FUNC(Std_ReturnType,FRIF_CODE) FrIf_GetNmVector
    (
        uint8 FrIf_CtrlIdx,
    ...
    )
{
    ...
    DBG_FRIF_GETNMVECTOR_ENTRY(FrIf_CtrlIdx,FrIf_NmVectorPtr);
...
/* check if development error detection is disabled */
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETNMVECTOR_SERVICE_ID, FRIF_E_NOT_INITIALIZED);
    }
    ...
    {
        /* Report to DET */
        FrIf_Det_ReportError(FRIF_GETNMVECTOR_SERVICE_ID, FRIF_E_INV_CTRL_IDX);
    }
...
#endif  /* FRIF_DEV_ERROR_DETECT == STD_OFF */
    {
        ...
        RetValue = FrIf_Call_Fr_Arg2(FRIF_FR_INSTANCE_IDX,GetNmVector,FRIF_FR_CTRLIDX, FrIf_NmVectorPtr);
    }

    /* return result */

    DBG_FRIF_GETNMVECTOR_EXIT(RetValue,FrIf_CtrlIdx,FrIf_NmVectorPtr);
    return RetValue;
}
